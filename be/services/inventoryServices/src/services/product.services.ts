...
const computeTrack = (item: PProduct): number => {
  const ageInDays =
    item.createdAt
      ...
      : 0;
  const velocity = (item.numPurchases ?? 0) / Math.max(ageInDays, 1);
  const newness = 1 / (1 + ageInDays / 14);
  return (
          ...
          $let: {
            ...
            in: {
              $add: [
                ...
                {
                  $multiply: [
                    { $divide: [{ $ifNull: ["$numPurchases", 0] }, { $max: ["$$ageInDays", 1] }] },
                    track.velocity,
                  ],
                },
                {
                  $multiply: [
                    { $divide: [1, { $add: [1, { $divide: ["$$ageInDays", 14] }] }] },
                    track.newness,
                  ],
                },
              ],
...
export const trackRecommendation = async (data: {
  ...
  if (idsNeedingType.length > 0) {
    const products = await Product.find({ _id: { $in: idsNeedingType } }).select("type");
    for (const p of products) {
      typeMap.set(p._id.toString(), p.type);
    }
  }
