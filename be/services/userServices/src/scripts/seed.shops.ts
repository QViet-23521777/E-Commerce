 ...
 *
 * Idempotent: upserts users by their deterministic `shopNN@seed.shop` email —
 * re-running never duplicates. Real accounts are untouched.
 *
...
async function main() {
  ...
  // Hash with argon2 — the login path (user.service.ts :: loginUser) verifies
