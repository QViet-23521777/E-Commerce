...
import { Eye, EyeOff, ArrowLeft, ShieldCheck, RotateCw } from "lucide-react";
...
export default function AdminLoginPage() {
  ...
  async function handleCredentials(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await apiRequest<AdminLoginResponse>("/api/admin/login", {
        method: "POST",
        body: { email, password },
      });
      // Email verification off → admin login already minted tokens; go in.
      if (res.data.twoFactorEnabled === false && res.tokens) {
        saveTokens(res.tokens.accessToken, res.tokens.refreshToken, "admin");
      ...
      setError(translateAdminError(e?.message) ?? "Invalid credentials");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleVerifyOtp(submittedOtp?: string) {
    ...
    setLoading(true);
    try {
      const res = await apiRequest<AdminTwoFAResponse>(
        "/api/admin/second-factor-auth",
        { method: "POST", body: { userId, otp: code } },
      );
      saveTokens(res.tokens.accessToken, res.tokens.refreshToken, "admin");
      window.location.assign("/admin/dashboard");
    } catch (err: unknown) {
      const e = err as { message?: string };
      setError(translateAdminError(e?.message) ?? "Verification failed. Try again.");
      setOtp("");
    } finally {
      setLoading(false);
    }
  ...
  async function handleResend() {
    ...
    try {
      ...
      const e = err as { message?: string };
      setError(translateAdminError(e?.message) ?? "Couldn't resend code.");
    } finally {
      setLoading(false);
    }
  }
          ...
          {step === "credentials" ? (
            ...
            >
              <div className="bg-surface-container-lowest border-2 border-deep-navy rounded-2xl overflow-hidden">
                ...
                <div className="px-8 py-10">
                  <div className="mb-8">
                    <div className="w-12 h-12 bg-red-50 border-2 border-red-300 rounded-2xl flex items-center justify-center mb-5">
                      <ShieldCheck className="w-5 h-5 text-red-500" />
                    </div>
                    <p className="text-label-caps text-red-500 mb-2">ADMIN VERIFICATION</p>
                    <h1 className="text-[2rem] font-bold leading-tight tracking-tight text-deep-navy mb-2">
                      Enter Your Code
                    </h1>
                    <p className="text-sm text-on-surface-variant">
                      We sent a 6-digit code to{" "}
                      <strong className="text-deep-navy break-all">{email}</strong>. It expires in 5 minutes.
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-6 pt-5 border-t border-outline-variant">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-semibold text-red-500 hover:underline underline-offset-2 disabled:text-on-surface-variant disabled:no-underline disabled:cursor-not-allowed"
                    >
...
function translateAdminError(msg: string | undefined): string | null {
  if (!msg) return null;
  switch (msg) {
