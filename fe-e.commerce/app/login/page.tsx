...
function LoginForm() {
  ...
  async function handleCredentials(e: React.FormEvent) {
    ...
    try {
      ...
      // When the account has email verification turned off, login already
      ...
      setError(translateAuthError(e?.message) ?? "Invalid email or password");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleVerifyOtp(submittedOtp?: string) {
    ...
    setLoading(true);
    try {
      const res = await apiRequest<TwoFAResponse>("/api/users/second-factor-auth", {
        method: "POST",
            ...
            >
              <div className="bg-surface-container-lowest border-2 border-deep-navy rounded-2xl overflow-hidden">
                ...
                <div className="px-8 py-10">
                  <div className="mb-8">
                    <p className="text-label-caps text-primary mb-2">WELCOME BACK</p>
                ...
                <div className="px-8 py-10">
                  <div className="mb-8">
                    ...
                    <p className="text-sm text-on-surface-variant">
                      ...
                      <strong className="text-deep-navy break-all">{email}</strong>. It
